...
export default function App() {
  ...
  const mergedData = useMemo(() => {
    ...
    for (const sub of approved) {
      ...
      const alreadyExists = result[dept].some(
        (s) => s.matricNo.trim().toLowerCase() === sub.matricNo.trim().toLowerCase()
      );
      if (!alreadyExists) {
        result[dept].push({ matricNo: sub.matricNo, name: sub.name, source: "dynamic" });
      }
    }
    return result;
  }, [approved]);
  ...
  // ── Print data: all departments including dynamic approved ──
      ...
      <div className="print-only" aria-hidden="true">
        <div className="print-title-block">
          ...
          <p className="print-subtitle">
            {totalCount} students &nbsp;·&nbsp; {printDepts.length} departments
          </p>
        ...
        <div className="print-footer">
          Total: {totalCount} students across {printDepts.length} departments
        </div>
      </div>

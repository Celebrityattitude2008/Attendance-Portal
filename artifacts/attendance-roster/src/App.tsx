...
export default function App() {
  ...
  const filtered = useMemo(() => {
    ...
    return DEPARTMENT_ORDER.reduce<Record<string, { matricNo: string; name: string }[]>>((acc, dept) => {
  ...
  const printDepts = DEPARTMENT_ORDER.filter((d) => (attendanceData[d]?.length ?? 0) > 0);
